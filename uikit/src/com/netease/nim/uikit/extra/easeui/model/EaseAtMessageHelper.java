package com.netease.nim.uikit.extra.easeui.model;
    ...
    public boolean containsAtUsername(String content){
        ...
        synchronized (toAtUserList) {
            for(String username : toAtUserList){
                ...
                if (NimUIKit.getUserInfoProvider().getUserInfo(username)!=null){
                    EaseUser user = (EaseUser) NimUIKit.getUserInfoProvider().getUserInfo(username);
                    if (user != null) {
                        nick = user.getNick();
                    }
                }
...
//                }
    ...
    public boolean isAtMeMsg(EMMessage message){
        EaseUser user = (EaseUser) NimUIKit.getUserInfoProvider().getUserInfo(message.getFrom());
        if(user != null){
            try {
