...
import com.netease.nim.uikit.session.custom.BoxingGlideLoader;
...
public abstract class PickImageAction extends BaseAction {
    ...
    protected PickImageAction(int iconResId, int titleId, boolean multiSelect) {
        ...
        config = new BoxingConfig(BoxingConfig.Mode.SINGLE_IMG); // Mode：Mode.SINGLE_IMG, Mode.MULTI_IMG, Mode.VIDEO
        config.needCamera().withMaxCount(9);
    }
...
//        showSelector(getTitleId(), requestCode, multiSelect, tempFile());
        Boxing.of(config).withIntent(getContainer().activity, BoxingActivity.class).start(getContainer().activity, PICK_IMAGE_COUNT);
    }
