...
import com.netease.nim.uikit.extra.delete.AppContextUtils;
...
public class DemoDBManager {
    ...
    private DemoDBManager(){
        dbHelper = DbOpenHelper.getInstance(AppContextUtils.get());
    }
    ...
    synchronized public void saveContactList(List<EaseUser> contactList) {
        ...
        if (db.isOpen()) {
            ...
            for (EaseUser user : contactList) {
                ...
                values.put(UserDao.COLUMN_NAME_ID, user.getAccount());
	...
	synchronized public void saveRobotList(List<RobotUser> robotList) {
		...
		if (db.isOpen()) {
			...
			for (RobotUser item : robotList) {
				ContentValues values = new ContentValues();
				values.put(UserDao.ROBOT_COLUMN_NAME_ID, item.getAccount());
