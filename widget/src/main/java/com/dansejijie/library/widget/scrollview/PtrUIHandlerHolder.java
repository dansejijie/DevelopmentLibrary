...
class PtrUIHandlerHolder implements PtrUIHandler {
    ...
    public void onUIReleaseToRefresh(ViewGroup frame) {
        ...
        do {
            final PtrUIHandler handler = current.getHandler();
            if (null != handler) {
                handler.onUIReleaseToRefresh(frame);
            }
        } while ((current = current.mNext) != null);
    }
    ...
    public void onUIRefreshFooterBegin(ViewGroup frame) {
        ...
        do {
            final PtrUIHandler handler = current.getHandler();
            if (null != handler) {
                handler.onUIRefreshFooterBegin(frame);
            }
        } while ((current = current.mNext) != null);
    }

    @Override
    public void onUIPositionChange(ViewGroup frame, boolean isUnderTouch, byte status, PtrIndicator ptrIndicator) {
        PtrUIHandlerHolder current = this;
        do {
            final PtrUIHandler handler = current.getHandler();
            if (null != handler) {
